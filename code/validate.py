...
chk("Audit fields complete (20 required manifest keys)", all(k in m for k in req))
chk("No post-hoc rescaling (free-point max radius before == after)", m["free_point_max_radius_before_finalization"]
    == m["free_point_max_radius_after_finalization"])
chk("Scale excludes centre (alpha computed without central anchor)", m["alpha_uses_central_anchor"] is False)
...
chk("Anchors pinned exactly (9 prescribed positions; row count == corpus size)", anch == expect and len(rows) == N_docs)

# 3) 測地線: 正式成功と実行深度 (nit>0)
geo = list(csv.DictReader(open(OUT + "/geodesic_results.csv")))
chk("Formal convergence (all geodesics report success)", len(geo) >= 30 and all(r["status"] == "success" for r in geo),
    f"n={len(geo)}")
import glob
nit_ok = True
for f in glob.glob(DATA + "/geo_*.json"):
    ...
    if not all(t["nit"] > 0 for t in r["tries"]):
        nit_ok = False
chk("Optimizer iterated (nit > 0 on every restart of every path)", nit_ok)
...
chk("Regression routes present (4 fixed centre-to-corner routes)", len(drops) == 4)
chk("Regression band (energy saving vs. straight line in 10-45%)", all(10 <= d <= 45 for d in drops),
    "drops=" + ",".join(f"{d:.1f}%" for d in drops))

# 5) GPR学習ゲート
g = json.load(open(OUT + "/gpr_info.json"))
chk("Multi-restart training (n_restarts = 10)", g.get("n_restarts") == 10)
chk("Hyperparameters moved (differ from initialization)", abs(g["length_scale_x"] - 0.5) > 1e-3 or abs(g["white_noise"] - 0.01) > 1e-3,
    f"ls=({g['length_scale_x']:.3f},{g['length_scale_y']:.3f}) noise={g['white_noise']:.3f}")
gz = np.load(DATA + "/grid_fields.npz")
chk("Field non-degenerate (max u >= 0.05)", float(gz["uncertainty"].max()) >= 0.05,
...
chk("Simplex closure (max |row sum - 1| < 1e-9, float64)", float(np.abs(rs - 1).max()) < 1e-9)
# 保存済み l1_sums が float64 和と整合するか (float32保存の丸め許容)
l1s = np.load(DATA + "/l1_sums.npy")
rel = np.abs(l1s - S.sum(axis=1)) / S.sum(axis=1)
chk("Stored normalizers (relative error vs. float64 sum < 1e-5)", float(rel.max()) < 1e-5)
...
chk("Reports present (7 required report files)", all(os.path.exists(OUT + "/" + f) for f in reports))
chk("Hash consistency (each report header carries the manifest SHA-256)", all(h in open(OUT + "/" + f).read() for f in reports))
